...
// Creates and returns a new [Query] with additional filter on specified [field]. [field] refers to a field in a document.
    FirebaseFirestore.instance.collection('String Collection Name').where('userId', isEqualTo: fbAuth.getUid).get().then(
...
=> To determine whether the document exists, use the exists property: */
    FirebaseFirestore.instance.collection('String Collection Name').doc('String document id').get().then((DocumentSnapshot documentSnapshot) {
...
// when it's no longer used within your app:
    final Stream<QuerySnapshot> collectionQueryStream = FirebaseFirestore.instance.collection('Collection name').snapshots();
...
// achievieng the same result
    final Stream<QuerySnapshot> collectionQueryStream1 = FirebaseFirestore.instance.collection('Collection name').snapshots();
    StreamBuilder<QuerySnapshot>(
...
// 'User-Agent': 'Thunder Client (https://www.thunderclient.com)', you don't need this line
        'Content-Type': 'application/json',
        'Authorization': 'key=AAAA3LFsk98:APA91bFIFB92REqC3Y5v9FqyqvJOQIW_9g4I9acdTyCVk13MjvqnbL9Dc94HFbcXeP2MIlAZzxI5bOFarhVCi1s5VBqtQAvBUk9pYFke -FTufTZDaO5JjnhYr0IVSwszwf4LvnMY-2Ij',
      };
